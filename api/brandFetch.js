// eslint-disable-next-line @typescript-eslint/no-var-requires
const axios = require("axios");
// eslint-disable-next-line @typescript-eslint/no-var-requires
const { createClient } = require("@supabase/supabase-js");

const supabaseUrl = process.env.REACT_APP_SUPABASE_URL;
const supabaseAnonKey = process.env.REACT_APP_SUPABASE_ANON_KEY;
const supabase = createClient(supabaseUrl, supabaseAnonKey);
const brandfetch_api_key = process.env.REACT_APP_BRANDFETCH;

module.exports = async (req, res) => {
  // Extract newRow from the parsed body
  const newRow = req.body;

  // Extract domainName from newRow
  const domainName = newRow?.record?.domain;

  // Log the domainName
  console.log("Domain name: ", domainName);

  // Check if the domainName parameter is an empty string
  if (!domainName) {
    res.status(400).json({ error: "Missing domainName parameter" });
    console.log("No domain name was passed into the function");
    return;
  }

  // Check if the domain name exists in the brandfetch_data table
  const { data, error } = await supabase
    .from("brandfetch_data")
    .select("domain")
    .eq("domain", domainName);

  if (error) {
    console.error(
      "Error fetching data from brandfetch_data table: ",
      error.message
    );
    res.status(500).json({ error: error.message });
    return;
  }

  // If the domain exists in the table, return without doing anything
  if (data.length > 0) {
    console.log(`Domain name ${domainName} already exists in the table.`);
    res.status(200).json({
      message: `Domain name ${domainName} already exists in the table.`,
    });
    return;
  }

  // If there is a domain and it doesn't exist in the brandfetch table, then retrieve it
  try {
    // Make a request to the Brandfetch API
    const response = await axios.get(
      `https://api.brandfetch.io/v2/brands/${domainName}`,
      {
        headers: {
          Authorization: `Bearer ${brandfetch_api_key}`,
        },
      }
    );

    console.log("Brandfetch Response:", response.data);

    // Upsert the response data into the brandfetch_data table
    const { error: upsertError } = await supabase
      .from("brandfetch_data")
      .upsert(response.data, { returning: "minimal" }); // don't return the inserted row

    if (upsertError) {
      console.log("Error upserting data:", upsertError.message);
      res
        .status(500)
        .send(`Error upserting brand data: ${upsertError.message}`);
      return;
    }

    // Send the response from the Brandfetch API
    res.status(200).send(response.data);
  } catch (error) {
    // Log the error message if the request failed
    console.log("Error fetching brand:", error.message);
    res.status(500).send(`Error fetching brand: ${error.message}`);
  }
};

// // eslint-disable-next-line @typescript-eslint/no-var-requires
// const axios = require("axios");
// // eslint-disable-next-line @typescript-eslint/no-var-requires
// const { createClient } = require("@supabase/supabase-js");
// // eslint-disable-next-line @typescript-eslint/no-var-requires
// const AWS = require("aws-sdk");
// AWS.config.update({
//   accessKeyId: process.env.REACT_APP_AWS_ACCESS_KEY_ID,
//   secretAccessKey: process.env.REACT_APP_AWS_SECRET_ACCESS_KEY,
//   region: "your-region", // replace 'your-region' with your AWS region
// });
// const sqs = new AWS.SQS();
// // eslint-disable-next-line @typescript-eslint/no-var-requires
// const { v4: uuidv4 } = require("uuid");

// const supabaseUrl = process.env.REACT_APP_SUPABASE_URL;
// const supabaseAnonKey = process.env.REACT_APP_SUPABASE_ANON_KEY;
// const supabase = createClient(supabaseUrl, supabaseAnonKey);
// const brandfetch_api_key = process.env.REACT_APP_BRANDFETCH;

// module.exports = async (req, res) => {
//   // Extract newRow from the parsed body
//   const newRow = req.body;

//   // Extract domainName from newRow
//   const domainName = newRow?.record?.domain;

//   // Define a jobId for this process.
//   const jobId = uuidv4(); // Generate a UUID

//   // Log the domainName
//   console.log("Domain name: ", domainName);

//   // Check if the domainName parameter is an empty string
//   if (!domainName) {
//     res.status(400).json({ error: "Missing domainName parameter" });
//     console.log("No domain name was passed into the function");
//     return;
//   }

//   // Check if the domain name exists in the brandfetch_data table
//   const { data, error } = await supabase
//     .from("brandfetch_data")
//     .select("domain")
//     .eq("domain", domainName);

//   if (error) {
//     console.error(
//       "Error fetching data from brandfetch_data table: ",
//       error.message
//     );
//     res.status(500).json({ error: error.message });
//     return;
//   }

//   // If the domain exists in the table, return without doing anything
//   if (data.length > 0) {
//     console.log(`Domain name ${domainName} already exists in the table.`);
//     res.status(200).json({
//       message: `Domain name ${domainName} already exists in the table.`,
//     });
//     return;
//   }
//   // Send a message to SQS indicating that the job has started
//   const startMessage = {
//     QueueUrl: "https://sqs.us-west-1.amazonaws.com/400399357295/brandFetchApi", // replace 'https://sqs.us-west-1.amazonaws.com/400399357295/brandFetchApi' with your SQS queue URL
//     MessageBody: JSON.stringify({
//       jobId,
//       status: "STARTED",
//       domainName,
//     }),
//   };
//   await sqs.sendMessage(startMessage).promise();

//   // If there is a domain and it doesn't exist in the brandfetch table, then retrieve it
//   try {
//     // Make a request to the Brandfetch API
//     const response = await axios.get(
//       `https://api.brandfetch.io/v2/brands/${domainName}`,
//       {
//         headers: {
//           Authorization: `Bearer ${brandfetch_api_key}`,
//         },
//       }
//     );

//     console.log("Brandfetch Response:", response.data);

//     // Upsert the response data into the brandfetch_data table
//     const { error: upsertError } = await supabase
//       .from("brandfetch_data")
//       .upsert(response.data, { returning: "minimal" }); // don't return the inserted row

//     if (upsertError) {
//       console.log("Error upserting data:", upsertError.message);
//       res
//         .status(500)
//         .send(`Error upserting brand data: ${upsertError.message}`);
//       return;
//     }

//     // Send the response from the Brandfetch API
//     res.status(200).send(response.data);

//     // After the function finishes processing, send a message to SQS indicating that the job has completed
//     const completeMessage = {
//       QueueUrl:
//         "https://sqs.us-west-1.amazonaws.com/400399357295/brandFetchApi", // replace 'https://sqs.us-west-1.amazonaws.com/400399357295/brandFetchApi' with your SQS queue URL
//       MessageBody: JSON.stringify({
//         jobId,
//         status: "COMPLETED",
//         domainName,
//       }),
//     };
//     await sqs.sendMessage(completeMessage).promise();
//   } catch (error) {
//     // Log the error message if the request failed
//     console.log("Error fetching brand:", error.message);
//     res.status(500).send(`Error fetching brand: ${error.message}`);

//     // Send a message to SQS indicating that the job has failed
//     const failedMessage = {
//       QueueUrl:
//         "https://sqs.us-west-1.amazonaws.com/400399357295/brandFetchApi", // replace 'https://sqs.us-west-1.amazonaws.com/400399357295/brandFetchApi' with your SQS queue URL
//       MessageBody: JSON.stringify({
//         jobId,
//         status: "FAILED",
//         domainName,
//         errorMessage: error.message,
//       }),
//     };
//     await sqs.sendMessage(failedMessage).promise();
//   }
// };
