...
export default function Dashboard() {
  ...
  const fetchAndCombineData = async (workspaceId, userId) => {
    ...
    // Iterate over the second result, adding each item to the Map.
    result2.data.forEach((item) => {
      combinedData.set(item.workspace_id, { ...item });
    });

    // Iterate over the first result, finding the corresponding item in the Map and merging the data.
